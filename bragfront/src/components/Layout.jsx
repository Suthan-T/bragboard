...
const Layout = () => {
    ...
    const navItems = user?.role === 'admin'
        ? [
            { path: '/admin-dashboard', label: 'Admin Dashboard', icon: <FaUserShield className="text-emerald-500" /> },
            { path: '/leaderboard', label: 'Leaderboard', icon: <FaTrophy className="text-yellow-500" /> },
                                        ...
                                        notifications.map(notif => (
                                            ...
                                            >
                                                <div className="flex-shrink-0" onClick={() => markAsRead(notif.id)}>
                                                    <img
                                                        src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${notif.actor?.name === 'Suthan' ? 'SuthanMale' : (notif.actor_id || 'sys')}`}
                                                        alt="User"
